...
export class TypeOrm{Entity}Repository implements {Entity}Repository {
  ...
  public async get{Entity}s(port: Get{Entity}sRepositoryPort): Get{Entity}RepositoryResult {
    const typeOrm{Entity}s: TypeOrm{Entity}[] = await this.{entity}Repository
      .createQueryBuilder("{entity}")
      .take(port.limit)
      .skip(port.offset)
      .getMany();

    const [typeOrm{Entity}s, totalItems]: [TypeOrm{Entity}[], number] = await query.getManyAndCount();
